...
#######################################################
#   FULL  Model Gaussian (model_repeat1)
#######################################################
...
spec_meantable1 <- data.frame(matrix(ncol = 4)) 
colnames(spec_meantable1) <- c('.chain','.iteration','.draw','b_spec_mean_cfFec')

logliktable1 <- matrix(NA, ncol=35)
...
for (i in 1:length(randomtrees)) {
  ...
  model_repeat1 <- brm(
    lg.eggsize ~ spec_mean_cfFec +(1|gr(spp_phylo, cov = A)) + (1|species_name)+(1|Location_name), 
    data = numeggs, 
    family = gaussian(), 
    data2 = list(A = A[[i]]),
    prior = c(
  ...
  fecundity1<- as.data.frame(spread_draws(model_repeat1, b_spec_mean_cfFec))
  
  intertable1 <- rbind(intertable1, intercept1)
  spec_meantable1 <- rbind(spec_meantable1, fecundity1)
